...
int main() {
  ...
  while(1)
  {
    if(i==primero.size() and j==segundo.size() and k==tercero.size())
    {
      break;
    }
    if(primero[i]==segundo[j] and i<primero.size() and j<segundo.size())
    {
      cout << primero[i];
      i++;
      j++;
    ...
    {
      if(primero[i]==tercero[k] and i<primero.size() and k<tercero.size())
      {
        cout << primero[i];
        i++;
        k++;
